...
// https://github.com/Servicerobotics-Ulm/OpcUaDeviceRepository/blob/master/OPCUAProductionStation/OpcUaProductionStation.cc: see createServerSpace()
...
private:
  ...
  virtual void handleMethodCall(const string &browseName, const vector<Variant> &inputs, vector<Variant> &outputs)
      override {
    if (inputs.size() == 1) {
...
int main(int argc, char *argv[]) {
  ...
  while (robot->step(timeStep) != -1) {
    ...
    isBoxPresent = distanceDevice->getValue() > 0.3;
    ...
    motorDevice0->setVelocity(motorSpeed * 0.3);
    motorDevice1->setVelocity(motorSpeed * 0.3);
